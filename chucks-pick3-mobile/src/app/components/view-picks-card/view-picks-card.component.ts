...
export class ViewPicksCardComponent implements OnInit {
  ...
  ngOnInit() {
    const numbersThatWereGenerated = this.drawTimeService
      .getCurrentDrawTimeCard()
      .getPick3DrawTimeArray();
    console.log(
      this.drawTimeService.currentDrawTimeCard.checkPick3DrawTimeArray
    );
    if (
      numbersThatWereGenerated !== null &&
      numbersThatWereGenerated !== undefined
    ) {
      ...
      this.toastService.presentToast(
        ...
        'Please generate numbers and come back later.',
        'results-not-available'
      );
    }
